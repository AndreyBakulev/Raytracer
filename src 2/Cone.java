/**
 * Represents a cone in 3D space. This shape is uncapped (you can cap with a ring or disc as required).
 *
 * @author Ben Farrar
 * @version 2022.05.12
 */
public class Cone extends Surface {
    ...
    public Intersection intersect(Ray ray){
        ...
        //Vector frameX = axis.cross(ray.getDirection()).normalize(); //this appears to be left-hand rule
        ...
        //MATH NOTE: We can ignore lots of arithmetic during projection by ensuring all frame vectors have length 1.
        ...
        //I lifted these straight from the source above, but ultimately it's because within this new reference frame,
        //  whenever x^2 + y^2 = r^2, THAT IS A HIT by definition!
        //So we go get the parameterized ray equations and just plug them into the formula above, and solve for t.
        //  That gets an equation in the form at^2 + bt + c = 0, which is solvable via the quadratic equation.
        //(radius/height) (aka 1/slope) is a scalar on z (and dz) in order to get the radius at that z
        //  Consider cone with h = 1 (and therefore at "base" z = 1), r = .5
        //  slope is 2, so r = z / slope
        
        //a=dx^2+dy^2-(dz/slope)^2, b=2x*dx+2y*dy-2z*dz/(slope^2), and c=x^2+y^2-(z/slope)^2
        ...
        double b = 2 * rayStartInFrame.getY() * rayMoveInFrame.getDy() - 2 * rayStartInFrame.getZ() * rayMoveInFrame.getDz() / Math.pow(slope,2);
        double c = Math.pow(rayStartInFrame.getX(),2) + Math.pow(rayStartInFrame.getY(),2) - Math.pow(rayStartInFrame.getZ() / slope,2);
        ...
        if (distanceInFrame < -EPSILON){
            //Specific check for being inside of a cone (first solution would be behind you)
        ...
        if(collisionInFrame.getZ() < 0 || collisionInFrame.getZ() > height){
            ...
            //  (e.g. looking "down the funnel" from the side: first collision is z>height, but second collision would be z<height.)
            distanceInFrame = ((-b)+Math.sqrt(determinant))/(2*a);
        ...
        //To calculate the normal, we can subtract off the position at the same height in the center of the cone,
        ...
        //If this is a collision with the inside of the cone, make sure the normal points inward as well
        if(normal.dot(ray.getDirection()) > 0){
            normal = normal.scale(-1);
        }
        return new Intersection(collision, normal, distanceInFrame, mat,0,0);
    }
}